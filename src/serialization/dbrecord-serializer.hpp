...
[[nodiscard]] bool serializeRecord(const Record& record, StorageIO<StorageImplementation>& io)
{
...
template<typename Record, typename StorageImplementation>
[[nodiscard]] bool deserializeRecord(Record& record, StorageIO<StorageImplementation>& io)
{
	// Statically sized fields are grouped before others and can be read or written in a single block.
	static constexpr size_t staticFieldsSize = record.staticFieldsSize();
	std::array<uint8_t, staticFieldsSize> buffer;
